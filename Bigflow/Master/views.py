        ...
        try:
            if(result=='"SUCCESS"' or result.replace('["','').replace('"]','').split(',')[-1]=='SUCCESS'):
                if (jsondata.get('Type') == 'State_Insert'):
                    ...
                    data = {'code':codes['code'],'name': data['State_name'], 'country_id': data['State_country_gid'],'Entity_Gid': Entity_gid,'create_by':Emp_gid}
                    mrobject = MasterRequestObject('STATE', data, 'POST')
                if (jsondata.get('Type') == 'District_Insert'):
                    data = jsondata.get('data').get('params').get('filter')
                    codes=get_data_from_id('DISTRICT',data)
                    data = {'code':codes['code'],'name': data['district_name'], 'state_code': codes['state_code'],'Entity_Gid': Entity_gid,'create_by':Emp_gid}
                    ...
                    city_data = {'code':codes['code'],'name': data['city_name'], 'state_code': codes['state_code'],'Entity_Gid': Entity_gid,'create_by':Emp_gid}
        ...
        if(df_cityset[0]=='SUCCESS'):
            try:
                if (jsondata.get('Params').get('t_name') == 'bank'):
                    data = {'code':jsondata.get('Params').get('t_code'),'name': jsondata.get('Params').get('t_tname'),'Entity_Gid':jsondata.get('Params').get('entity_gid'),'create_by':jsondata.get('Params').get('create_by')}
