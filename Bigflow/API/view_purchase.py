        ...
        try:
            if self.request.query_params.get("Type") == "pr_multi":
                ...
                if ld_out_message.get("MESSAGE") == 'SUCCESSFULLY UPDATED':
                    ...
                    from Bigflow.Core.models import get_data_from_id as gdfi
                    from Bigflow.Core.models import delmat_tran_to_type
                    if(jsondata['delmat_status']=='Approved'):
                        codes = gdfi('DELMAT_APPROVE', jsondata)
                        print(codes)
                        dtype=delmat_tran_to_type(codes['tran'])
                        data = {'Entity_Gid':ent_gid,'create_by':self.request.query_params.get("Employee_Gid"),"employee_code":codes['employee_code'],"commodity_code":codes['commodity_code'], "delmat_type":dtype}
                        mrobject = MasterRequestObject('DELMAT_APPROVE', data, 'POST')
                    if (jsondata['delmat_status'] == 'Rejected'):
                        codes = gdfi('DELMAT_APPROVE', jsondata)
                        print(codes)
                        dtype = delmat_tran_to_type(codes['tran'])
                        data = {'Entity_Gid':ent_gid,'create_by':self.request.query_params.get("Employee_Gid"),"employee_code":codes['employee_code'],"commodity_code":codes['commodity_code'], "delmat_type":dtype}
                        mrobject = MasterRequestObject('DELMAT_REJECT', data, 'POST')
                elif ld_out_message.get("MESSAGE") == 'FAIL':
                    ld_dict = {"MESSAGE": ld_out_message.get("MESSAGE")}
                else:
