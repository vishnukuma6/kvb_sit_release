...
class eClaim_Approve(APIView):
    def post(self,request):
        try:
            ...
            if jsondata.get('Params').get('DETAILS').get('apptype') == 'CLAIM' and jsondata.get('Params').get(
                ...
                try:
                    ...
                    if emp_out_message.get("MESSAGE") == 'FOUND':
                            ...
                            if int(a) > 3:
                                for i in employee_tmp_data:
                                    if int(i.get('employee_grade1')[1]) >= int(a):
                                        if i.get('branch_code') in appr_branch:
                                            Expense_Approve = True
                                        else:
                                            ld_dict = {"MESSAGE": 'Please Choose 1101,2135,1618,1260,1299 Branch Approver', "STATUS": 1}
                                            return Response(ld_dict)

                                    else:
                                        ld_dict = {"MESSAGE": 'Please Choose Higher Grade Approver', "STATUS": 1}
                                        return Response(ld_dict)
                            else:
                                Expense_Approve = True
                        else:
                        ...
                        ld_dict = {"MESSAGE": 'Employee Data Missing'+ str(ld_out_message.get("MESSAGE")), "STATUS": 1}
                        return Response(ld_dict)
                except Exception as e:
                    common.logger.error(e)
                    ld_dict = {
                        "MESSAGE": "Please Choose Higher Grade Approver/ Please Choose 1101,2135,1618,1260,1299 Branch Approver", "STATUS": 1}
                    return Response(ld_dict)
            ...
            if jsondata.get('Params').get('DETAILS').get('apptype') == "TOURADV" and jsondata.get('Params').get(
                ...
                if Expense_Approve:
                    ...
                    ld_dict = {
                        "MESSAGE": "Please Choose Higher Grade Approver/ Please Choose 1101 or 2135 Branch Approver"}
                    return Response(ld_dict)
...
class Approver_List(APIView):
    def get(self,request):
        try:
            ...
            br = request.query_params.get("Branch_Gid")
            apptype = request.query_params.get("App_Type")
            if apptype == 'TOURADV' or apptype == 'TOUR':
                obj_claim.filter_json = json.dumps({"App_Type": apptype, "Entity_Gid": entity, "Branch_Gid": br})
                ...
                if ld_out_message.get("MESSAGE") == 'FOUND':
                    ...
                    if emp_out_message.get("MESSAGE") == 'FOUND':
                        ...
                        ld_dict = {"MESSAGE": "Employee" + str(ld_out_message.get("DATA"))}
                        return Response(ld_dict)
                    # emp_data = {
                    #     "empids": employeegid
                    # }
            ...
            if ld_out_message.get("MESSAGE") == 'FOUND':
                ...
                if len(data) != 0:
                    ...
                    for d in data:
                        ...
                        d["branch_code"] = employee_data[0].get('branch_code')
                        d["branch_name"] = employee_data[0].get('branch_name')
                        d["designation_name"] = employee_data[0].get('designation_name')
                    if permit_gid != []:
                        for d in data:
                            for i in range(0, len(permit_data)):
                                if permit_data[i].get('employee_gid') == int(d.get('approvedby')):
...
def lodging_logic(self,empid,json_param):
    tourgid = json_param.get('Params').get('FILTER').get('tourgid')
    checkindate = json_param.get('Params').get('FILTER').get('checkindate')
    checkoutdate = json_param.get('Params').get('FILTER').get('checkoutdate')
    grade = emp_grade(tourgid)
...
class Pkgmvg_Logic(APIView):
    def post(self, request):
        try:
            ...
            entity_gid = bank_data[0].get('entity_gid')
            json_param = android.Android.param_conversion(self, jsondata, entity_gid)
            eligible = pkgmvg_logic(self,empid,json_param,entity_gid)
            ld_dict = {"DATA": eligible,
                       "MESSAGE": 'FOUND'}
            return Response(ld_dict)
        except Exception as e:
...
def pkgmvg_logic(self,empid,json_param,entity_gid):
    tourgid = json_param.get('Params').get('FILTER').get('tourgid')
    grade = emp_grade(tourgid)
    eligible_value = verify.pkg_moving.validate_data(self, json_param, grade,empid,entity_gid)
    common.logger.error([{"eligible_value": str(eligible_value)}])
    driverbatta = 0
    daysdrivereng = 0
    traveltimeinhours = ""
    if 'traveltimeinhours' in json_param.get('Params').get('FILTER'):
        traveltimeinhours = int(json_param.get('Params').get('FILTER').get('traveltimeinhours'))
    else:
        traveltimeinhours = 0
    if  traveltimeinhours != 0 or traveltimeinhours != "0":
        ...
        try:
            ...
            if request.query_params.get("Api_Type") == "WEB":
                ...
                for i in CHANGE:
                    json_param = android.Android.param_conversion(self, i, entity)
                    json_param['Params']['FILTER']['expensegid'] = DETAILS.get('expensegid')
                    ld_dict = pkgmvg_logic(self, empid, json_param,entity)
    ...
    def post(self, request):
        try:
            ...
            if request.query_params.get("Api_Type") == "WEB":
                jsondata = json.loads(request.body.decode('utf-8'))
                obj_claim.filter_json = json.dumps(jsondata.get('Params').get('FILTER'))
                ld_out_message = obj_claim.eClaim_file_remove_set()
            else:
                jsondata = json.loads(request.body.decode('utf-8'))
                obj_claim.filter_json = json.dumps(jsondata)
                ld_out_message = obj_claim.eClaim_file_remove_set()
                ...
                for d in data:
                    for i in range(0, len(employee_data)):
                        if employee_data[i].get('employee_gid') == int(d.get('empgid')):
                            d["employee_name"] = employee_data[i].get('employee_name')
                            d["employee_code"] = employee_data[i].get('employee_code')
                            d["employee_designation"] = employee_data[i].get('designation_name')
                            d["branch_name"] = employee_data[i].get('branch_name')
                            d["branch_code"] = employee_data[i].get('branch_code')
                            break
...
class Approvedby_data(APIView):
    def post(self,request):
        try:
            ...
            obj_claim = meClaim.eClaim_Model()
            obj_claim.jsondata = json.dumps(jsondata)
            ld_out_message = obj_claim.eClaim_approvedbydata_set()
            if ld_out_message.get("MESSAGE") == 'SUCCESS':
                ld_dict = {"MESSAGE": "SUCCESS", "STATUS": 0}
            elif ld_out_message.get("MESSAGE") == 'FAIL':
                ld_dict = {"MESSAGE": "FAIL", "STATUS": 1}
