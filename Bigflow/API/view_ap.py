...
class crn_get(APIView):
    def post(self, request):
        try:
            crn_obj = mAP.ap_model()
            jsondata = json.loads(request.body.decode('utf-8'))
            crn_obj.action = self.request.query_params.get("action")
            # crn_obj.type = request.query_params.get("type")
            crn_obj.filter = json.dumps(jsondata.get('Params').get('filter'))
            crn_obj.classification = json.dumps({"Entity_Gid": jsondata.get('Params').get("entity_gid")})
            # ecf_obj.create_by = request.query_params.get("employee_gid")
            # common.main_fun1(request.read(), path)
            ld_out_message = crn_obj.get_crn_details()
            if ld_out_message.get("MESSAGE") == 'FOUND':
                ld_dict = {"DATA": json.loads(ld_out_message.get("DATA").to_json(orient='records')),
                           "MESSAGE": 'FOUND'}
            elif ld_out_message.get("MESSAGE") == 'NOT_FOUND':
