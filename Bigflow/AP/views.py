...
def Invoiceheader_Mail_set(request):
    ...
    if request.method == 'POST':
        try:
            ...
            email.send()
            if (jsondata.get('params').get('action') == 'UPDATE' and jsondata.get('params').get('type') == 'MAIL_STATUS'):
                try:
                    inward_dtl.action = jsondata.get('params').get('action')
                    inward_dtl.type = jsondata.get('params').get('type')
                    inward_dtl.header_json = jsondata.get('params').get('header_json')
    ...
    if request.method == 'POST':
        try:
            ...
            if invoice_get.action == 'INVOICE_DETAILS_EDIT':
                ...
                df_credit = (
                out[['credit_gid', 'credit_paymodegid', 'credit_refno', 'bankbranch_ifsccode', 'bankdetails_bank_gid',
    ...
    if request.method == 'POST':
        try:
            ...
            if (action == "GET" and type == "ONWARD_SALES"):
                ...
                jdata = out.to_json(orient='records')
                return JsonResponse(json.loads(jdata), safe=False)
            elif (action == "GET" and (type == "AP_ENTER_GET" or type == "ECF_TRANS_GET" or type=="AP_WISEFIN_ENTER_GET")):
                invoice_set.action = jsondata.get('params').get('action')
                invoice_set.type = jsondata.get('params').get('type')
                filter = jsondata.get('params').get('filter')
                invoice_set.filter = json.dumps(filter)
    ...
    if request.method == 'GET':
        try:
            ...
            if (action == "GET" and type == "ENTRY_QUERY"):
                ...
                final_df = df_view[['entry_refno','entry_transactiondate','entry_gl','gl_name','entry_amt','entry_type',
                                    'supplier_branchname','branch_name','entry_module']]
                final_df.columns = ['ENTRY_REFNO','ENTRY_TRANSACTION_DATE','ENTRY_GL','ENTRY_GL_NAME','ENTRY_AMOUNT','ENTRY_TYPE',
                ...
                jsondata = {'action': action,'type': type,
                            'filter': {"InvoiceHeader_Status": txn_status,"InvoiceHeader_Gid":"","invoiceheader_invoicetype":txn_type,
                ...
                final_df = df_view[
                    ['S_No','branch_name','branch_code','branchgst','supplier_code','supplier_branchname',
                     ...
                     'invoiceheader_status','invoiceheader_captalisedflag','invoiceheader_rcmapplicable','subcategory_gstblocked',
                ...
                final_df.columns = ['S. No','Branch','Branch Code','Branch GST','Vendor Code',
                                    ...
                                    'Goods/Services GST Rate','Goods/Services GL Code','Goods/Services UoM','Goods/Services Qty',
                                    ...
                                    'Transaction Status','Capitalisation Flag','RCM Applicability Tag','Is the credit blocked',
                                    'Total Credit Available CGST','Total Credit Available SGST','Total Credit Available IGST','Total Credit Available Total',
                                    'Credit Expensed CGST','Credit Expensed SGST','Credit Expensed IGST','Credit Expensed Total',
                                    'Credit Available CGST','Credit Available SGST','Credit Available IGST','Credit Available Total',
                ...
                final_df = df_view[
                    ['S_No','branch_name','branch_code','branchgst','supplier_code','supplier_branchname',
                     ...
                     'invoiceheader_invoicedate','invoicedetails_item','invoicedetails_amount','invoicedetails_hsncode',
                     'taxrate','debit_glno','temp_var','invoicedetails_qty',
                     'invoicedetails_unitprice','invoicedetails_cgst','invoicedetails_sgst','invoicedetails_igst',
                     'temp_var','invoiceheader_roundoff','invoiceheader_deductionamount','invoicedetails_totalamt',
                     'invoiceheader_crno','paymentheader_date','invoiceheader_invoicetype','invoicedetails_desc',
                ...
                final_df = df_view[
                    ['invoiceheader_crno', 'invoiceheader_invoicetype','invoiceheader_status','employee_name',
                     ...
                     'invoiceheader_invoiceno','invoiceheader_invoicedate','supplier_name','supplier_code','supplier_branchname',
                ...
                jsondata = {'action': action, 'type': type,
                            'filter': {"ecf_number":InvoiceHeader_CRNo,"invoiceheader_invoicetype":InvoiceHeader_Type,
                                       "employee_name": invoiceheader_Employee_gid,"invoiceheader_invoiceno":InvoiceHeader_InvoiceNo,"ap_status":InvoiceHeader_AP_Status,
                                       "ecf_status": InvoiceHeader_ECF_Status, "supplier_name": supplier_name,"branch_gid":invoiceheader_branchgid,
                                       "from_date":fromdate,"to_date":todate,"min_amount":InvoiceHeader_Min_Amt,"max_amount":InvoiceHeader_Max_Amt}}
                ...
                jsondata = {'action': action, 'type': type,
                            'filter': {"ecf_number": InvoiceHeader_CRNo, "invoice_type": InvoiceHeader_Type,"employee_gid":employee_gid,
                                       ...
                                       "max_amount": InvoiceHeader_Max_Amt, "invoiceheader_status": InvoiceHeader_Status}}
        ...
        try:
            ...
            if (jsondata.get('params').get('status_json').get('Status') != 'APPROVED'):
                try:
                    ...
                    result_data=""
                    header_gid=""
                    if(jsondata.get('params').get('action') == 'INSERT' and jsondata.get('params').get('type') == 'INVOICE_HEADER'):
                    ...
                    if(jsondata.get('params').get('action') == 'INSERT' and
                        ...
                        try:
                            ...
                            inward_dtl.classification = json.dumps({"Entity_Gid":inward_dtl.entity_gid, "Emp_gid":inward_dtl.employee_gid})
                            templates_data = inward_dtl.get_multiple_email_templates_data()
                            Mail_Data = templates_data.get("Mail_Data")[0].get("mailtemplate_body")
                            Header_Data = templates_data.get("Header_Data")[0]
                            for (k, v) in Header_Data.items():
                                                                ...
                                                                if (len(Final_Unique_Values) == len(Unique_Values)):
                                                                    if(Is_approve_and_pay=="Y"):
                                                                        try:
                                                                            ...
                                                                            filter = {"InvoiceHeader_Status": "APPROVED", "InvoiceHeader_Gid": [invoice_header_gid],
                                                                                      "InvoiceHeader_InvoiceDate": "", "fromdate": "",
                                                                                      "invoiceheader_branchgid": "", "Page_Index": 0, "Page_Size": 10}
                                                                            invoice_set.filter = json.dumps(filter)
                                                                            invoice_set.classification = json.dumps({"Entity_Gid":entity_gid})
                                                                            invoice_set.create_by = employee_gid
                                                                            out = invoice_set.get_invoice_all()
                                                                            ...
                                                                            for i in jdata:
                                                                                ...
                                                                                if(i.get("paymode_name")!="CREDITGL"):
                                                                                    Payment_Mode=i.get("paymode_name")
                                                                                    paymode_glno=i.get("paymode_glno")
                                                                                    credit_ddtranbranch=i.get("credit_ddtranbranch")
                                                                                    credit_ddpaybranch=i.get("credit_ddpaybranch")
                                                                                    bankbranch_name=i.get("bankbranch_name")
                                                                                    bankdetails_acno=i.get("bankdetails_acno")
                                                                                    ifsccode=i.get("bankbranch_ifsccode")
                                                                                    beneficiaryname=i.get("bankdetails_beneficiaryname")
                                                                                    bank_name = i.get("bank_name")
                                                                                    invoiceheader_remarks=i.get("invoiceheader_remarks")
                                                                                    invoiceheader_gid=i.get("invoiceheader_gid")
                                                                                    invoiceheader_status=i.get("invoiceheader_status")
                                                                                    invoiceheader_crno=i.get("invoiceheader_crno")
                                                                                    invoiceheader_invoiceno=i.get("invoiceheader_invoiceno")
                                                                                    credit_bankgid=i.get("credit_bankgid")
                                                                                if(i.get("invoiceheader_ppx")=="E"):
                                                                                    Ref_id =i.get("invoiceheader_employeegid")
                                                                                    Header_type = "EMPLOYEE_PAYMENT"
                                                                                elif(i.get("invoiceheader_ppx")=="I"):
                                                                                    Ref_id=i.get("invoiceheader_branchgid")
                                                                                    Header_type = "BRANCH_PAYMENT"
                                                                                else:
                                                                                    Ref_id = i.get("supplier_gid")
                                                                                    Header_type = "SUPPLIER_PAYMENT"

                                                                                Paymentheader_Amount = Paymentheader_Amount + i.get("credit_amount")
                                                                            Paymentheader_Amount = round(Paymentheader_Amount, 2)
                                                                            if(Payment_Mode==""):
                                                                            ...
                                                                            pass_data={'params': {'action': 'Insert', 'type': 'PAYMENT_ADD',
                                                                                                  'header_json': {'HEADER': [{'Paymentheader_Amount':Paymentheader_Amount , 'Payment_Mode':Payment_Mode,
                                                                                                                              'paymode_glno':paymode_glno, 'credit_ddtranbranch': credit_ddtranbranch,"Paymentheader_Status": "PAYMENT INITIATE",
                                                                                                                              'credit_ddpaybranch':credit_ddpaybranch,"bankbranch_name":bankbranch_name,"Remark": "PAYMENT INITIATE",
                                                                                                                              "Bank_Detail_Gid":credit_bankgid,"Paymentheader_Date":datenow,"REF_Gid":Ref_id,"Header_type":Header_type,
                                                                                                                              "bankdetails_acno":bankdetails_acno,"ifsccode":ifsccode,"beneficiaryname":beneficiaryname}]},
                                                                                                  'detail_json': {'DETAILS':jdata },
                                                                                                  'other_json': {}, 'status_json': {}}}
                                                                            return APpayment_set_function(pass_data,employee_gid,entity_gid)
                        ...
                        if(ap_statu_update_result=="SUCCESS"):
                            try:
                                                                                ...
                                                                                if (Is_approve_and_pay == "Y"):
                                                                                    try:
                                                                                        ...
                                                                                        header_update_data = {
                                                                                            ...
                                                                                            "type": "ERROR_UPDATE",
                                                                                            "employee_gid": employee_gid,
                                                                                            "Error_log": {"MESSAGE": "ERROR_OCCURED_ON_AP_PAYMENTDETAILS_GET_","DATA": str(e)}}
                                                                                        AP_Header_Update_result = AP_Header_Update(header_update_data)
                                                                                        #pass
                                                                                        # return JsonResponse({"MESSAGE": "ERROR_OCCURED_ON_PAYMENT_FUNCTION","DATA": str(e)})
                                                                                else:
                                ...
                                header_update_data = {"Invoice_Header_Gid": invoice_header_gid,
                                                      "entity_gid": entity_gid,
                                                      "type": "ERROR_UPDATE", "employee_gid": employee_gid,
                                                      "Error_log": {"MESSAGE": "ERROR_OCCURED_ON_ACCOUNTING_ENTRY_INITIAL_SET_",
                            ...
                            header_update_data = {"Invoice_Header_Gid": invoice_header_gid, "entity_gid": entity_gid,
                                                  "type": "ERROR_UPDATE", "employee_gid": employee_gid,
                                                  "Error_log": {"MESSAGE": "ERROR_OCCURED_ON_AP INITIATED_UPDATION","OUT":ap_statu_update_result}}
                            AP_Header_Update_result = AP_Header_Update(header_update_data)
                            # return JsonResponse({"MESSAGE": "ERROR_OCCURED_ON_AP INITIATED_UPDATION","OUT":ap_statu_update_result})
                    except Exception as e:
                        common.logger.error(e)
                ...
                for i in range(len(header_json_data)):
                    try:
                        ...
                        if (Payment_Mode == "NEFT"):
                            ...
                            NEFT_Header_Amount = round(header_data_df.loc[header_data_df['paymode_name'] == 'NEFT', 'credit_amount'].sum(),2)
                            if (bankbranch_name == None or bankdetails_acno == None or ifsccode == None and beneficiaryname == None):
                                execute = 0
                                Header_Status_Data = {"invoiceheader_crno": invoiceheader_crno,"ERROR_MESSAGE":"MISSING_BANK_DETAILS"}
                                Final_Header_Status_Data.append(Header_Status_Data)
                        if(Payment_Mode=="DD"):
                            header_data_df = pd.DataFrame(detail_json_data)
                            DD_Header_Amount = round(header_data_df.loc[header_data_df['paymode_name'] == 'DD', 'credit_amount'].sum(), 2)
                            if (beneficiaryname == None):
                                execute = 0
                                Header_Status_Data = {"invoiceheader_crno": invoiceheader_crno,"ERROR_MESSAGE":"MISSING_BENEFICIARY_NAME"}
                                Final_Header_Status_Data.append(Header_Status_Data)
                            elif (Payment_Mode_GL == None):
                                execute = 0
                                Header_Status_Data = {"invoiceheader_crno": invoiceheader_crno,"ERROR_MESSAGE":"MISSING_PAYMODE_GL_DETAILS"}
                                Final_Header_Status_Data.append(Header_Status_Data)
                            elif(Credit_DDpay_Branch==None or Credit_DDtran_Branch==None or Credit_DDpay_Branch=='0' or Credit_DDtran_Branch=='0'):
                                execute = 0
                            ...
                            try:
                                ...
                                if (out == "SUCCESS" and jsondata.get("params").get('type') == "PAYMENT_ADD"):
                                    try:
                                        ...
                                        inward_dtl.type = "AP_ENTER_DETAIL_GET"
                                        # ref_no = jsondata.get('params').get('status_json').get('ref_no')
                                        inward_dtl.filter = json.dumps({"entry_refno":invoiceheader_crno,"entry_module":"PAYMENT"})
                                        inward_dtl.classification = json.dumps({"Entity_Gid": entity_gid})
                                        datas = inward_dtl.get_invoice_all()
                                        Fund_Transfer_Dtls_data = datas.get("entry_detail")[0]
                                        Fund_Transfer_Dtls_data = json.loads(Fund_Transfer_Dtls_data)
                                                ...
                                                if (Payment_Mode== "NEFT"):
                                                    ...
                                                    try:
                                                        ...
                                                        if token_status == 1:
                                                            try:
                                                                ...
                                                                log_data = [{"AFTER_NEFT_PAYEMENT_API_DATA": neft_results_data}]
                                                                common.logger.error(log_data)
                                                                if(neft_results_data.get("ErrorMessage")=="Success" and neft_results_data.get("ErrorCode")=='0'):
                                                                    ...
                                                                    try:
                                                                        ...
                                                                        inward_dtl.employee_gid = employee_gid
                                                                        final_out = outputSplit(inward_dtl.set_Invoiceheader_status_update(), 1)
                                                                        return JsonResponse({"MESSAGE": "ERROR_OCCURED_ON_NEFT_API","DATA": neft_results_data,"UPDATE_STATUS":final_out})
                                                                    except Exception as e:
                                                                        return JsonResponse({"MESSAGE": "ERROR_OCCURED_ON_NEFT_NOT_SUCCESS_AND_STATUS_UPDATE","DATA": str(e)})
                                                            except Exception as e:
                                                                common.logger.error(e)
                                                                return JsonResponse({"MESSAGE": "ERROR_OCCURED_ON_NEFT_API","DATA": str(e)})
                                                        else:
                                                            return JsonResponse({"MESSAGE": "ERROR_OCCURED_ON_TOKEN_GENERATIONS OR INVALID AUTHENTICATONS","TOKEN_LOG": log_data})
                                                    except Exception as e:
                                                        try:
                                                            ...
                                                            inward_dtl.employee_gid = employee_gid
                                                            final_out = outputSplit(inward_dtl.set_Invoiceheader_status_update(), 1)
                                                            return JsonResponse({"MESSAGE": "ERROR_OCCURED_ON_NEFT_LOCAL_API","DATA": str(e),"UPDATE_STATUS":final_out})

                                                        except Exception as e1:
                                                            common.logger.error(e1)
                                                            return JsonResponse({"MESSAGE": "ERROR_OCCURED_ON_NEFT_LOCAL_API_AND_STATUS_UPDATE_FAILED","DATA": str(e1)})
                                                    ...
                                                    try:
                                                        ...
                                                        if (dd_statu_update_result == "SUCCESS"):
                                                            ...
                                                            return JsonResponse({"MESSAGE": "ERROR_OCCURED_ON_DD_INITIATED_UPDATION","OUT": dd_statu_update_result})
                                                    except Exception as e:
                                                        common.logger.error(e)
                                                        return JsonResponse({"MESSAGE": "ERROR_OCCURED_ON_DD_INITIATED_UPDATION","DATA": str(e)})
                                                            ...
                                                            try:
                                                                ...
                                                                if(dd_results_data.get("ErrorMessage")=="Success" and dd_results_data.get("ErrorCode")=='0'):
                                                                    try:
                                                                        ...
                                                                        common.logger.error(e)
                                                                        return JsonResponse({"MESSAGE": "ERROR_OCCURED_ON_DD_SUCCESS_STATUS_UPDATE","DATA": str(e)})
                                                                else:
                                                                    try:
                                                                        inward_dtl.action = "UPDATE"
                                                                        inward_dtl.type = "STATUS"
                                                                        ...
                                                                        inward_dtl.employee_gid = employee_gid
                                                                        final_out = outputSplit(inward_dtl.set_Invoiceheader_status_update(), 1)
                                                                        return JsonResponse({"MESSAGE": "ERROR_OCCURED_ON_DD_API","DATA": dd_results_data,"UPDATE_STATUS":final_out})
                                                                    except Exception as e:
                                                                        common.logger.error(e)
                                                                        return JsonResponse({"MESSAGE": "ERROR_OCCURED_ON_DD_NOT_SUCCESS_AND_STATUS_UPDATE","DATA": str(e)})
                                                            except Exception as e:
                                                                try:
                                                                    inward_dtl.action = "UPDATE"
                                                                    inward_dtl.type = "STATUS"
                                                                    ...
                                                                    inward_dtl.employee_gid = employee_gid
                                                                    final_out = outputSplit(inward_dtl.set_Invoiceheader_status_update(), 1)
                                                                    return JsonResponse({"MESSAGE": "ERROR_OCCURED_ON_DD_API","DATA": str(e),"UPDATE_STATUS":final_out})

                                                                except Exception as e1:
                                                                    common.logger.error(e1)
                                                                    return JsonResponse({"MESSAGE": "ERROR_OCCURED_ON_DD_LOCAL_API_AND_STATUS_UPDATE_FAILED","DATA": str(e1)})
                                                        else:
                                                            return JsonResponse({"MESSAGE": "ERROR_OCCURED_ON_TOKEN_GENERATIONS OR INVALID AUTHENTICATONS","TOKEN_LOG": log_data})
                                                    except Exception as e:
                                                        common.logger.error(e)
                                                        return JsonResponse({"MESSAGE": "ERROR_OCCURED_ON_DD_API", "DATA": str(e)})
                                                    ...
                                                    try:
                                                        ...
                                                        common.logger.error(e)
                                                        return JsonResponse({"MESSAGE": "ERROR_OCCURED_AMOUNT_TRANSFER_SUCCESS_FINAL_STATUS_UPDATE_FAILED","DATA": str(e)})
                                            else:
    ...
    try:
        ...
        if (jsondata.get('params').get('action') == "Insert" and jsondata.get('params').get('type') == "PAYMENT_ADD"):
                        ...
                        if (ap_statu_update_result == "SUCCESS"):
                            ...
                            if (execute == 1):
                                try:
                                    ...
                                    inward_dtl.entity_gid = entity_gid
                                    inward_dtl.employee_gid = employee_gid
                                    common.logger.error([{"BEFORE_PAYMENT_SET_CR_NUMBER": invoiceheader_crno, "LEVEL": 2}])
                                    out_put_data = inward_dtl.set_payment()
                                    common.logger.error([{"AFTER_PAYMENT_SET_CR_NUMBER": invoiceheader_crno,"OUTPUT":out_put_data, "LEVEL": 2}])
                                    out_data = out_put_data[0].split(",")
                                    pv_number = out_data[0]
                                    out = out_data[1]
                                    if (out == "SUCCESS" and jsondata.get("params").get('type') == "PAYMENT_ADD"):
                                        try:
                                            ...
                                            try:
                                                ...
                                                common.logger.error([{"GET_CBSDATE_ACCOUNT_NUMBER_TRY":str(e)}])
                                                    ...
                                                    if (invoiceheader_status != "AMOUNT TRAN"):
                                                        ...
                                                        if token_status == 1:
                                                            try:
                                                                ...
                                                                SECOND_AMOUNT_TRN_START_TIME_D = datetime.now()
                                                                SECOND_AMOUNT_TRN_START_TIME = SECOND_AMOUNT_TRN_START_TIME_D.strftime("%d/%m/%Y %H:%M:%S")
                                                                ...
                                                                SECOND_AMOUNT_TRN_END_TIME_D = datetime.now()
                                                                SECOND_AMOUNT_TRN_END_TIME = SECOND_AMOUNT_TRN_END_TIME_D.strftime("%d/%m/%Y %H:%M:%S")
                                                                SECOND_AMOUNT_TRN_TIME_DIFF = SECOND_AMOUNT_TRN_END_TIME_D - SECOND_AMOUNT_TRN_START_TIME_D
                                                                SECOND_AMOUNT_TRN_TIME_DIFF_DATA = divmod(SECOND_AMOUNT_TRN_TIME_DIFF.days * seconds_in_day + SECOND_AMOUNT_TRN_TIME_DIFF.seconds,60)
                                                                second_amount_trn_log_data = [{
                                                                                          "SECOND_AMOUNT_TRN_START_TIME": SECOND_AMOUNT_TRN_START_TIME,
                                                                                          "SECOND_AMOUNT_TRN_END_TIME": SECOND_AMOUNT_TRN_END_TIME,
                                                                                          "SECOND_AMOUNT_TRN_TIME_DIFF_DATA": SECOND_AMOUNT_TRN_TIME_DIFF_DATA}]
                                                                ...
                                                                if (ErrorStatus == "Success" and ErrorCode == '0'):
                                                                    ...
                                                                    if (Payment_Mode == "KVBAC" or Payment_Mode == "BRA" or Payment_Mode == "ERA"):
                                                                        ...
                                                                        try:
                                                                            ...
                                                                            common.logger.error([{"ERROR_OCCURED_ON_AMOUNT_TRN_SUCCESS_STATUS_UPDATE_TRY":str(e)}])
                                                                            header_update_data = {
                                                                                "Invoice_Header_Gid": invoiceheader_gid,
                                                                                "entity_gid": entity_gid,
                                                                                "type": "ERROR_UPDATE",
                                                                ...
                                                                common.logger.error([{"ERROR_OCCURED_ON_AMOUNT_TRANSFER_API_TRY":str(e)}])
                                                                header_update_data = {
                                                                    "Invoice_Header_Gid": invoiceheader_gid,
                                                                    "entity_gid": entity_gid,
                                                                    "type": "ERROR_UPDATE",
                                                ...
                                                if (amount_transfer_message == "SUCCESS"):
                                                    if (Payment_Mode == "NEFT"):
                                                        ...
                                                        try:
                                                            ...
                                                            common.logger.error([{"ERROR_OCCURED_ON_NEFT_INITIATED_UPDATION":str(e)}])
                                                            header_update_data = {
                                                                "Invoice_Header_Gid": invoiceheader_gid,
                                                                "entity_gid": entity_gid,
                                                                "type": "ERROR_UPDATE",
                                                        ...
                                                        try:
                                                            ...
                                                            log_data_c = [{"BEFORE_NEFT_PAYEMENT_API_DATA": data2}]
                                                            common.logger.error(log_data_c)
                                                            ...
                                                            try:
                                                                ...
                                                                common.logger.error([{"ERROR_OCCURED_ON_TOKEN_GENERATION_NEFT_TOKEN_TRY":str(e)}])
                                                                # return JsonResponse({"MESSAGE": "ERROR_OCCURED_ON_TOKEN_GENERATION","DATA": str(e)})
                                                                pass
                                                            log_data_d = generated_token_data
                                                            token = generated_token_data.get("DATA")[0].get(
                                                                "clienttoken_name")
                                                            if (token == " " or token == None):
                                                                token_status = 0
                                                            if token_status == 1:
                                                                try:
                                                                    ...
                                                                    common.logger.error(log_data_e)
                                                                    if (neft_results_data.get(
                                                                        ...
                                                                        try:
                                                                            ...
                                                                            common.logger.error([{"ERROR_OCCURED_ON_NEFT_NOT_SUCCESS_AND_STATUS_UPDATE_TRY": str(e)}])
                                                                            header_update_data = {
                                                                                "Invoice_Header_Gid": invoiceheader_gid,
                                                                                "entity_gid": entity_gid,
                                                                                "type": "ERROR_UPDATE",
                                                                    ...
                                                                    common.logger.error([{"ERROR_OCCURED_ON_NEFT_API_TRY":str(e)}])
                                                                    header_update_data = {
                                                                        "Invoice_Header_Gid": invoiceheader_gid,
                                                                        "entity_gid": entity_gid,
                                                                        "type": "ERROR_UPDATE",
                                                                ...
                                                                common.logger.error("ERROR_OCCURED_ON_TOKEN_GENERATIONS_OR_INVALID_AUTHENTICATONS")
                                                            ...
                                                            try:
                                                                ...
                                                                common.logger.error([{"ERROR_OCCURED_ON_NEFT_API_AND_STATUS_UPDATE_FAILED_TRY":str(e)}])
                                                                header_update_data = {
                                                                    "Invoice_Header_Gid": invoiceheader_gid,
                                                                    "entity_gid": entity_gid, "type": "ERROR_UPDATE",
                                                                    "employee_gid": employee_gid,
                                                        ...
                                                        try:
                                                            ...
                                                            common.logger.error([{"ERROR_OCCURED_ON_DD_INITIATED_UPDATION_TRY":str(e)}])
                                                            header_update_data = {
                                                                "Invoice_Header_Gid": invoiceheader_gid,
                                                                "entity_gid": entity_gid,
                                                                "type": "ERROR_UPDATE", "employee_gid": employee_gid,
                                                        ...
                                                        try:
                                                            log_data_f = [{"BEFORE_DD_PAYEMENT_API_DATA": data3}]
                                                            common.logger.error(log_data_f)
                                                            data = json.dumps(data3)
                                                            token_status = 1
                                                            try:
                                                                generated_token_data = master_views.master_sync_Data_(
                                                                ...
                                                                common.logger.error([{"ERROR_OCCURED_ON_TOKEN_GENERATION_PAYEMNT_DD_TRY":str(e)}])
                                                            log_data = generated_token_data
                                                            token = generated_token_data.get("DATA")[0].get(
                                                                "clienttoken_name")
                                                            if (token == " " or token == None):
                                                            ...
                                                            if token_status == 1:
                                                                try:
                                                                    ...
                                                                    common.logger.error(log_data_g)
                                                                    if (dd_results_data.get(
                                                                        ...
                                                                        try:
                                                                            ...
                                                                            common.logger.error([{"ERROR_OCCURED_ON_DD_NOT_SUCCESS_AND_STATUS_UPDATE_TRY":str(e)}])
                                                                            header_update_data = {
                                                                                "Invoice_Header_Gid": invoiceheader_gid,
                                                                                "entity_gid": entity_gid,
                                                                                "type": "ERROR_UPDATE",
                                                                    ...
                                                                    common.logger.error([{"ERROR_OCCURED_ON_DD_API_TRY": str(e)}])
                                                                    try:
                                                                        inward_dtl.action = "UPDATE"
                                                                        inward_dtl.type = "STATUS"
                                                                        inward_dtl.header_json = '{}'
                                                                        ...
                                                                        common.logger.error([{"ERROR_OCCURED_ON_DD_LOCAL_API_AND_STATUS_UPDATE_FAILED_TRY":str(e1)}])
                                                                        header_update_data = {
                                                                            "Invoice_Header_Gid": invoiceheader_gid,
                                                                            "entity_gid": entity_gid,
                                                                            "type": "ERROR_UPDATE",
                                            ...
                                            common.logger.error([{"ERROR_OCCURED_AP_ENTER_DETAIL_GET_TRY":str(e)}])
                                            header_update_data = {"Invoice_Header_Gid": invoiceheader_gid,
                                                                  "entity_gid": entity_gid,
                                                                  "type": "ERROR_UPDATE", "employee_gid": employee_gid,
                                                                  "Error_log": {
                                    ...
                                    common.logger.error([{"ERROR_OCCURED_ON_PAYMENT_INSERT_TRY":str(e)}])
                                    header_update_data = {"Invoice_Header_Gid": invoiceheader_gid,
                                                          "entity_gid": entity_gid,
                                                          "type": "ERROR_UPDATE", "employee_gid": employee_gid,
                            ...
                            header_update_data = {"Invoice_Header_Gid": invoiceheader_gid, "entity_gid": entity_gid,
                                                  "type": "ERROR_UPDATE", "employee_gid": employee_gid,
                                                  "Error_log": {"MESSAGE": "ERROR_OCCURED_ON_PAY_INITIATED_UPDATION", "OUT": ap_statu_update_result}}
                            AP_Header_Update_result = AP_Header_Update(header_update_data)
                            #pass
                            # return JsonResponse({"MESSAGE": "ERROR_OCCURED_ON_AP INITIATED_UPDATION", "OUT": ap_statu_update_result})
                    except Exception as e:
                        common.logger.error([{"ERROR_OCCURED_ON_PAY_INITIATED_UPDATION_TRY":str(e)}])
                        header_update_data = {"Invoice_Header_Gid": invoiceheader_gid, "entity_gid": entity_gid,
                                              "type": "ERROR_UPDATE", "employee_gid": employee_gid,
                                              "Error_log":{"MESSAGE": "ERROR_OCCURED_ON_PAY_INITIATED_UPDATION_", "DATA": str(e)}}
                        AP_Header_Update_result = AP_Header_Update(header_update_data)
                        # return JsonResponse({"MESSAGE": "ERROR_OCCURED_ON_AP INITIATED_UPDATION", "DATA": str(e)})
                except Exception as e:
                    common.logger.error(e)
                    header_update_data = {"Invoice_Header_Gid": invoiceheader_gid, "entity_gid": entity_gid,"type": "ERROR_UPDATE", "employee_gid": employee_gid,
                                          "Error_log":{"MESSAGE": "ERROR_OCCURED_", "DATA": str(e)}}
                    AP_Header_Update_result = AP_Header_Update(header_update_data)
                    #return JsonResponse({"MESSAGE": "ERROR_OCCURED_ON_BANK_DETAILS_GET", "DATA": str(e)})
                    #pass
            ...
            try:
                ...
                common.logger.error([{"PAYMENT_SET_TRY":str(e)}])

    except Exception as e:
        common.logger.error("PAYMENT_FUNCTION_COMMON_TRY")

#@sched.interval_schedule(minutes=1)
#@sched.interval_schedule(seconds=1)
#sched.approve_and_pay_auto_load(minute=1)
class approve_and_pay_with_class:
    def __init__(self, status):
        self.running_status = status
    def approve_and_pay_auto_load(self):
        try:
            log_data1=[]
            log_data2=[]
            log_data3=[]
            ...
            log_data7=[]
            log_data8=[]
            first_amount_trn_log_data=[]
            seconds_in_day = 24 * 60 * 60;
            # self.running_status != "running"
            if(1):
                now = datetime.now()
                SCHEDULER_INITATED = now.strftime("%d/%m/%Y %H:%M:%S")
                log_data = [{"SCHEDULER_INITATED_TIME": SCHEDULER_INITATED,"AP_SCHEDULER_WORKING_IP":socket.gethostbyname(socket.gethostname())}]
                common.logger.error(log_data)
                try:
                    obj_ap = mAP.ap_model()
                    ...
                    common.logger.error("ERROR_FLAG_UPDATE_CALL")
                    ld_out_message = obj_ap.ap_update_flag()
                    common.logger.error([{"ERROR_FLAG_UPDATE_RESULT":str(ld_out_message)}])
                    try:
                        inward_dtl = mAP.ap_model()
                        inward_dtl.action = "GET"
                        ...
                        common.logger.error("TRANSACTION_INITIATED_CALL")
                        ...
                        common.logger.error([{"TRANSACTION_INITIATED_DATA":json_data}])
                        data_len=len(json_data)
                        count=0;
                        SCHEDULER_START_TIME=""
                        INVOICE_START_TIME_D=""
                        INVOICE_START_TIME=""
                        if(len(json_data)!=0):
                            for i in json_data:
                                self.running_status='running'
                                ...
                                if(count==1):
                                    now = datetime.now()
                                    SCHEDULER_START_TIME=now.strftime("%d/%m/%Y %H:%M:%S")
                                    log_data1 = [{"SCHEDULER_START_TIME":SCHEDULER_START_TIME ,"TOTAL_RECORDS":data_len}]
                                    common.logger.error(log_data1)
                                try:
                                    inward_dtl_c = mAP.ap_model()
                                    inward_dtl_c.action = "GET"
                                    inward_dtl_c.type = "TRANSACTION_INITIATED_CHECK"
                                    inward_dtl_c.filter = json.dumps({"invoiceheader_gid":i.get("invoiceheader_gid")})
                                    inward_dtl_c.classification = json.dumps({"Entity_Gid": 1})
                                    common.logger.error([{"TRANSACTION_INITIATED_CHECK_WITH_GID":i.get("invoiceheader_gid")}])
                                    single_invoice_status_data = inward_dtl_c.get_invoice_all()
                                    single_invoice_status_data_all = json.loads(single_invoice_status_data.to_json(orient='records'))
                                    common.logger.error([{"TRANSACTION_INITIATED_CHECK_OUT_PUT":single_invoice_status_data_all}])
                                    if(single_invoice_status_data_all[0].get("invoiceheader_status")=="TRANSACTION INITIATED"):
                                        now = datetime.now()
                                        INVOICE_START_TIME_D = now
                                        INVOICE_START_TIME = INVOICE_START_TIME_D.strftime("%d/%m/%Y %H:%M:%S")
                                        log_data2 = [{"INVOICE_START_TIME": INVOICE_START_TIME, "CR_NUMBER": i.get("invoiceheader_crno")}]
                                        common.logger.error(log_data2)

                                        inward_dtl = mAP.ap_model()
                                        jsondata ={'params': {'action': 'UPDATE', 'type': 'STATUS', 'header_json': {}, 'debit_json': {}, 'detail_json': {},
                                        'status_json': {'Invoice_Header_Gid': i.get("invoiceheader_gid"), 'Status': 'APPROVED', 'Header_Status': 'CHECKER',
                                        'Invoice_Type':i.get("invoiceheader_invoicetype"), 'Is_approve_and_pay': 'Y', 'file_data': [],
                                        'ref_no': i.get("invoiceheader_crno"), 'branch_gid':i.get("invoiceheader_branchgid")}}}
                                        if (jsondata.get('params').get('action') == 'UPDATE' and
                                                jsondata.get('params').get('type') == 'STATUS' and
                                                jsondata.get('params').get('status_json').get('Status') == 'APPROVED'):
                                            try:
                                                ...
                                                try:
                                                    ...
                                                    if (ap_statu_update_result == "SUCCESS"):
                                                        try:
                                                            ...
                                                            inward_dtl.filter = json.dumps({"InvoiceHeader_Gid": invoice_header_gid,
                                                                                            ...
                                                                                            "Invoice_Type": Invoice_Type})
                                                            inward_dtl.classification = json.dumps({"Entity_Gid": entity_gid})
                                                            common.logger.error([{"BEFORE_SET_ACCOUNTING_ENTRY_CR_NUMBER":cr_no,"LEVEL":1}])
                                                            inward_dtl.create_by = employee_gid
                                                            out = outputSplit(inward_dtl.set_accounting_entry(), 1)
                                                            common.logger.error([{"CR_NUMBER":cr_no,"AFTER_SET_ACCOUNTING_ENTRY_OUT_PUT":out,"LEVEL":1}])
                                                            if (out == 'SUCCESS' or out == 'ALREADY INSERTED'):
                                                                try:
                                                                    inward_dtl.action = "GET"
                                                                    inward_dtl.type = "AP_ENTER_DETAIL_GET"
                                                                            ...
                                                                            if (send_Credit_amount == send_Debit_amount):
                                                                                ...
                                                                                try:
                                                                                    ...
                                                                                    generated_token_data = master_views.master_sync_Data_(
                                                                                        ...
                                                                                        "get_data",
                                                                                        employee_gid)
                                                                                    log_data3 = generated_token_data
                                                                                    token = generated_token_data.get("DATA")[0].get(
                                                                                        "clienttoken_name")
                                                                                    if (token == " " or token == None):
                                                                                        token_status = 0
                                                                                    if token_status == 1:
                                                                                        try:
                                                                                            ...
                                                                                            FIRST_AMOUNT_TRN_START_TIME = FIRST_AMOUNT_TRN_START_TIME_D.strftime("%d/%m/%Y %H:%M:%S")
                                                                                            ...
                                                                                            FIRST_AMOUNT_TRN_END_TIME_D = datetime.now()
                                                                                            FIRST_AMOUNT_TRN_END_TIME = FIRST_AMOUNT_TRN_END_TIME_D.strftime("%d/%m/%Y %H:%M:%S")
                                                                                            FIRST_AMOUNT_TRN_TIME_DIFF = FIRST_AMOUNT_TRN_END_TIME_D - FIRST_AMOUNT_TRN_START_TIME_D
                                                                                            FIRST_AMOUNT_TRN_TIME_DIFF_DATA = divmod(FIRST_AMOUNT_TRN_TIME_DIFF.days * seconds_in_day + FIRST_AMOUNT_TRN_TIME_DIFF.seconds,60)
                                                                                            first_amount_trn_log_data = [{"FIRST_AMOUNT_TRN_START_TIME":FIRST_AMOUNT_TRN_START_TIME,"FIRST_AMOUNT_TRN_END_TIME":FIRST_AMOUNT_TRN_END_TIME,
                                                                                                                   "FIRST_AMOUNT_TRN_TIME_DIFF_DATA": FIRST_AMOUNT_TRN_TIME_DIFF_DATA}]
                                                                                            common.logger.error(first_amount_trn_log_data)
                                                                                            ...
                                                                                            if (
                                                                                                ...
                                                                                                try:
                                                                                                        ...
                                                                                                        if (len(
                                                                                                            ...
                                                                                                            if (
                                                                                                                ...
                                                                                                                try:
                                                                                                                    ...
                                                                                                                    common.logger.error([{"AP_PAYMENTDETAILS_TRY":str(e)}])
                                                                                                                    header_update_data = {
                                                                                                                        "Invoice_Header_Gid": invoice_header_gid,
                                                                                                                        "entity_gid": entity_gid,
                                                                                                                        "type": "ERROR_UPDATE",
                                                                                                    ...
                                                                                                    common.logger.error({"ERROR_OCCURED_ON_INVOICE_HEADER_UPDATE_TRY":str(e)})
                                                                                                    header_update_data = {
                                                                                                        "Invoice_Header_Gid": invoice_header_gid,
                                                                                                        "entity_gid": entity_gid,
                                                                                                        "type": "ERROR_UPDATE",
                                                                                            ...
                                                                                            common.logger.error([{"ERROR_OCCURED_ON_AMOUNT_TRANSFER_CLINET_API_TRY":str(e)}])
                                                                                            header_update_data = {
                                                                                                "Invoice_Header_Gid": invoice_header_gid,
                                                                                                "entity_gid": entity_gid,
                                                                                                "type": "ERROR_UPDATE",
                                                                                    ...
                                                                                    common.logger.error([{"ERROR_OCCURED_TOKEN_GENERATION_TRY":str(e)}])
                                                                                    header_update_data = {
                                                                                        "Invoice_Header_Gid": invoice_header_gid,
                                                                                        "entity_gid": entity_gid,
                                                                                        "type": "ERROR_UPDATE",
                                                                    ...
                                                                    common.logger.error([{"ERROR_OCCURED_ON_ENTRY_DETAILS_GET_TRY":str(e)}])
                                                                    header_update_data = {"Invoice_Header_Gid": invoice_header_gid,
                                                                                          "entity_gid": entity_gid,
                                                                                          "type": "ERROR_UPDATE",
                                                                                          "employee_gid": employee_gid,
                                                            ...
                                                            common.logger.error([{"ERROR_OCCURED_ON_ACCOUNTING_ENTRY_INITIAL_SET_TRY":str(e)}])
                                                            header_update_data = {"Invoice_Header_Gid": invoice_header_gid,
                                                                                  "entity_gid": entity_gid,
                                                                                  "type": "ERROR_UPDATE", "employee_gid": employee_gid,
                                                                                  "Error_log": {
                                                    ...
                                                    common.logger.error([{"AP_INITIATED_TRY":str(e)}])
                                                    header_update_data = {"Invoice_Header_Gid": invoice_header_gid,
                                                                          "entity_gid": entity_gid,
                                                                          "type": "ERROR_UPDATE", "employee_gid": employee_gid,
                                                                          "Error_log": {"MESSAGE": "ERROR_OCCURED_ON_", "DATA": str(e)}}
                                                ...
                                                common.logger.error([{"AP_BEFORE_INITIATED_TRY":str(e)}])
                                                header_update_data = {"Invoice_Header_Gid": invoice_header_gid,
                                                                      "entity_gid": entity_gid, "type": "ERROR_UPDATE",
                                                                      "employee_gid": employee_gid,
                                                                      "Error_log": {"MESSAGE": "ERROR_OCCURED_", "DATA": str(e)}}
                                            ...
                                            common.logger.error("INVALID AP STATUS")
                                    else:
                                        common.logger.error([{"TRANSACTION_INITIATED_CHECK_TRY_FAIL":i.get("invoiceheader_crno")}])
                                except Exception as e:
                                    common.logger.error([{"TRANSACTION_INITIATED_CHECK_TRY":str(e)}])

                                now = datetime.now()
                                INVOICE_END_TIME_D=now
                                INVOICE_END_TIME = INVOICE_END_TIME_D.strftime("%d/%m/%Y %H:%M:%S")
                                INVOICE_START_END_TIME_DIFF = INVOICE_END_TIME_D - INVOICE_START_TIME_D
                               # INVOICE_TIME_DIFF=str(((INVOICE_END_TIME_D.minute)-(INVOICE_START_TIME_D.minute)))+":"+str(((INVOICE_END_TIME_D.second)-(INVOICE_START_TIME_D.second)))
                                INVOICE_TIME_DIFF=divmod(INVOICE_START_END_TIME_DIFF.days * seconds_in_day + INVOICE_START_END_TIME_DIFF.seconds, 60)
                                log_data6 = [{"INVOICE_START_TIME":INVOICE_START_TIME,
                                             "INVOICE_END_TIME": INVOICE_END_TIME,
                                             "INVOICE_TIME_DIFF":INVOICE_TIME_DIFF,
                                             "CR_NUMBER": i.get("invoiceheader_crno")}]
                                common.logger.error(log_data6)
                            if (count == data_len):
                                now = datetime.now()
                                log_data7 = [{"SCHEDULER_START_TIME":SCHEDULER_START_TIME,"SCHEDULER_END_TIME": now.strftime("%d/%m/%Y %H:%M:%S"),
                                             "TOTAL_RECORDS":data_len}]
                                common.logger.error(log_data7)
                            self.running_status=''
                        else:
                            common.logger.error([{"TRANSACTION_INITIATED_DATA":json_data,"MESSAGE":"NO RECORDS FOUND"}])
                    except Exception as e:
                        common.logger.error([{"TRANSACTION_INITIATED_TRY":str(e)}])
                except Exception as e:
                    common.logger.error([{"UPDATE_ERROR_FLAG_TRY": str(e)}])
            else:
                now = datetime.now()
                SCHEDULER_RUNNING_WITH_EXIT_TIME = now.strftime("%d/%m/%Y %H:%M:%S")
                log_data8 = [{"SCHEDULER_RUNNING_WITH_EXIT": SCHEDULER_RUNNING_WITH_EXIT_TIME}]
                common.logger.error(log_data8)
        except Exception as e:
            common.logger.error([{"COMMON_TRY":str(e)}])

#ap_object=approve_and_pay_with_class('start')
#sched.add_interval_job(ap_object.approve_and_pay_auto_load, minutes=10,max_instances=1)
    ...
    if request.method == 'POST':
        try:
            ...
            if(type=="INVOICE_HEADER" and html_file_status==0):
                return JsonResponse({"MESSAGE": "ERROR_OCCURED","DATA":{"ECF_CREATION_STATUS":out,"NOTES_INSERT_STATUS":html_file_except}})
            return JsonResponse(out, safe=False)
        except Exception as e:
            return JsonResponse({"MESSAGE": "ERROR_OCCURED", "DATA": str(e)})
        ...
        try:
            ...
            for i in result_data:
                try:
                    ...
                    ammort_set.action = action
                    ammort_set.type = type
                    ammort_set.filter = json.dumps({"DETAILS": {"AmortDetails_Gid": i.get('ammortdetail_amortheadergid'),
            ...
            common.logger.error(e)
            return JsonResponse({"MESSAGE": "ERROR_OCCURED","ERROR_MESSAGE":"ERROR ON AMORT DETAILS GET","DATA": str(e)})
