...
class ap_model(mFET.FET_model):
    ...
    def get_invoice_all(self):
        ...
        common.logger.error([{"SP":"sp_APProcess_Get","TRANSACTION_INITIATED_MODLE":parameters}])
    ...
    def ap_update_flag(self):
        cursor = connection.cursor()
        parameters = (self.action, self.type, self.filter, self.classification, self.create_by, '');
        common.logger.error([{"SP":"sp_AP_Update_Status","ERROR_FLAG_MODEL":parameters}])
    ...
    def get_crn_details(self):
        cursor = connection.cursor()
        parameters = (self.action,self.filter, self.classification, '');
        cursor.callproc('sp_CRnumber_Get', parameters)
        columns = [x[0] for x in cursor.description]
        rows = cursor.fetchall()
        rows = list(rows)
        cursor.execute('select @_sp_CRnumber_Get_3')
        outmsg_sp = cursor.fetchone()
        cursor.close()
