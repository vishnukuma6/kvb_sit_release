...
def localip():
    try:
        ...
        logger.error([{"APP_URL":"No name APP_URL in env"}])

def ip_address_validate():
    try:
        return env.str("Two_Factor_Auth_KVB_Whitelisted_IPs")
    except:
        logger.error([{"Two_Factor_Auth_KVB_Whitelisted_IPs": "No name Two_Factor_Auth_KVB_Whitelisted_IPs in env"}])

def two_factor_authenticate():
    try:
        return env.str("Two_Factor_Auth_Status")
    except:
        logger.error([{"Two_Factor_Auth_Status": "No name Two_Factor_Auth_Status in env"}])
...
def server_environ_var():
    try:
        ...
        logger.error([{"SERVER_ENVIRONMENT": "No name SERVER_ENVIRONMENT in env"}])
...
def clientapi():
    try:
        ...
        logger.error([{"API_CLIENT_URL": "No name API_CLIENT_URL in env"}])
...
def memoapi_url():
    try:
        ...
        logger.error([{"MEMO_API_URL": "No name MEMO_API_URL in env"}])
...
def master_accesstoken():
    try:
        ...
        logger.error([{"API_CLIENT_URL_ACCESSTOKEN": "No name API_CLIENT_URL in env"}])
...
def master_sync():
    try:
        ...
        logger.error([{"API_CLIENT_URL_MASTERSYNC": "No name API_CLIENT_URL in env"}])
...
def senderemail():
    try:
        ...
        logger.error([{"TO_EMAIL": "No name TO_EMAIL in env"}])
...
def ADToken():
    try:
        ...
        logger.error([{"API_CLIENT_ID": "No name API_CLIENT_ID in env"}])
...
def ClientSecret():
    try:
        ...
        logger.error([{"API_CLIENT_SECRET": "No name API_CLIENT_SECRET in env"}])


def Apiname():
    try:
        return env.str("HRD_API_NAME")
    except:
        logger.error([{"HRD_API_NAME": "No name HRD_API_NAME in env"}])


def ApiPassword():
    try:
        return env.str("HRD_API_PASSWORD")
    except:
        logger.error([{"HRD_API_PASSWORD": "No name HRD_API_PASSWORD in env"}])
...
class MasterRequestObject():
    ...
    all_master_urls = {
        ...
        'PROCUREMENT': {
            ...
            'COMMODITY_MAP':"prserv/cpmapping_code",
            ...
            'DELMAT_APPROVE':'prserv/delmatapprove_mtom',
            'DELMAT_REJECT':'prserv/delmatreject_mtom'
        },
        'TAX MASTER':{
            'TAX':'mstserv/taxmtom',
            ...
            if(self.execute):
                try:
                    ...
                    if (respo.status_code != 200):
                        ...
                        parameter_json['status'] = status
                    try:
                        if(resp_obj['code'] == "UNEXPECTED_ERROR" or resp_obj['code']=='INVALID_COMMODITYPRODUCTMAPING_ID'):
...
def schedule_apiRun():
    ...
    for row in rows:
        ...
        for data in row:
            ...
            if(mst_name=="COMMODITY_MAP"):
                print(str(records)[-20:-1])
                records=str(records[:-20])+'"'+str(records[-20:-1])+'"'+str(records[-1])
