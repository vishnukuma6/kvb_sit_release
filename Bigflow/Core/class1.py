...
class login(mVariable.variable):
    ...
    def authreauest_set(self):
        cursor = connection.cursor()
        Parameters = ('GET', 'INSERT_UPDATE',self.emp_json, self.emp1_json, '')
        cursor.callproc('sp_Auth_Request_Get', Parameters)
        cursor.execute('select @_sp_Auth_Request_Get_4')
        df_header = cursor.fetchone()
        return df_header[0]

    def checkauthreauest_set(self):
        cursor = connection.cursor()
        Parameters = ('DATE_GET', 'DATE_RETRIEVE',self.emp_json, self.emp1_json, '')
        cursor.callproc('sp_Auth_Request_Get', Parameters)
        cursor.execute('select @_sp_Auth_Request_Get_4')
        df_header = cursor.fetchone()
        return df_header[0]
