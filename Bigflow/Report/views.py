...
def Auto_PPR_Set_schdule():
    ...
    common.logger.error([{"BEFORE_PPRDATA_AUTO_START": {"PPR_Auto_Schdule": str(datetime.now().strftime("%Y-%m-%d %H:%M:%S")),"Response":""}}])
...
def PPR_Budget_Get_Summary(request):
    if request.method == 'POST':
        try:
            jsondata = json.loads(request.body.decode('utf-8'))
            if (jsondata.get('Group')) == 'SUMMARY' or "FinYear_Fetch" or "Sector_Fetch" or "Business_Fetch" or "Bs_Fetch" or "CC_Fetch" or "Expense_grp":
                grp = jsondata.get('Group')
                action = jsondata.get('Action')
                typ = jsondata.get('Group')
                params = {'Group': "" + grp + "", 'Action': "" + action + "", 'Type': "" + typ + ""}
        ...
        try:
            ...
            if column_arr == list(excel_data_DF.columns):
                Json_data_XL = excel_data_DF.drop(
                    ...
                    "Budget Amount": "Amount","Financial Year":"Fin_year","Quarter (in nos)":"Quarter","Month (in numarical)":"trn_month"
    ...
    if request.method == 'POST':
        try:
            ...
            if (jsondata.get('Group')) == 'Budget_Insert':
                ...
                response = resp.content.decode("utf-8")
                return HttpResponse(response)
            if (jsondata.get('Group')) == 'Reviewer_Insert':
                grp = jsondata.get('Group')
                action = jsondata.get('Action')
                typ = jsondata.get('Group')
                params = {'Group': "" + grp + "", 'Action': "" + action + "", 'Type': "" + typ + ""}
                ...
                headers = {"content-type": "application/json", "Authorization": "" + token + ""}
                datas = json.dumps(jsondata.get('data'))
                resp = requests.post("" + ip + "/PPR_Reviewer_Set", params=params, data=datas, headers=headers,
                                     verify=False)
                response = resp.content.decode("utf-8")
                return HttpResponse(response)
            if (jsondata.get('Group')) == 'Reviewer_Get':
                grp = jsondata.get('Group')
                action = jsondata.get('Action')
                typ = jsondata.get('Group')
                params = {'Group': "" + grp + "", 'Action': "" + action + "", 'Type': "" + typ + ""}
                ...
                headers = {"content-type": "application/json", "Authorization": "" + token + ""}
                datas = json.dumps(jsondata.get('data'))
                resp = requests.post("" + ip + "/PPR_Reviewer_Get", params=params, data=datas, headers=headers,
                                     verify=False)
                response = resp.content.decode("utf-8")
                return HttpResponse(response)
