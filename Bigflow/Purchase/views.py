...
def commo_product(request):
    ...
    if request.method == 'POST':
        ...
        if df_customer_ddl[0]=='SUCCESS': #Commodity_Product_Mapping Sync
            from Bigflow.Core.models import MasterRequestObject
            from Bigflow.Core.models import get_data_from_id as gdfi
            print(jsondata,json.dumps(df_customer_ddl))
            params=jsondata['data']['Params']['FILTER']
            commo_map_code=gdfi('COMMODITY_PROD_MAP',params)
            print(commo_map_code)
            parameter_data={"commodity_code":commo_map_code['commodity_code'],"product_code":json.loads(commo_map_code['product_code']),"Entity_Gid": request.session['Entity_gid'],"create_by":request.session['Emp_gid']}
            mrobject=MasterRequestObject('COMMODITY_MAP',parameter_data,'POST')
...
def commosave(request):
    ...
    if request.method == 'POST':
        ...
        Employee_gid = decry_data(request.session['Emp_gid'])
        entity_gid = decry_data(request.session['Entity_gid'])
        obj_producategory_ddl.value2 = json.dumps({'Entity_Gid':entity_gid,'Create_By':Employee_gid,'Entity_Gid_sync':request.session['Entity_gid'],'Create_By_sync':request.session['Emp_gid']})
