...
class utility():
    ...
    def check_login(request):
        try:
            obj_check_detail = class1.login()
            if 'Login_Gid' not in request.session:
                return utility.error_403(request)
            obj_check_detail.entity_gid = decry_data(request.session['Entity_gid'])
            obj_check_detail.detail_gid = request.session['Login_Gid']
            out_msg = obj_check_detail.get_checklogin()
            common.logger.error([{"LG_Check_Logins_msg": out_msg}])
            if out_msg == 'NOT_FOUND':
                return request
            elif (out_msg == 'FOUND'):
                common.logger.error(' Another Login Found for  ' + str(
                    decry_data(request.session['Emp_gid'])) + ' - Employee Name = ' +
                                    request.session['Emp_name'])
                request.session.flush()
                request.session['Emp_name'] = 'dummy'
