    ...
    if request.method == 'POST':
        ...
        if jsondata.get('Type') == "AP_Category":
            ...
            drop_b = {
                ...
                "Column_1": "assetdetails_id,assetdetails_gid,assetdetails_subcat,assetdetails_branchgid",
                "Column_2": "",
            ...
            response = alltable(request, drop_b, entity)
            return HttpResponse(response)
        elif jsondata.get('Type') == "Asset_Branch":
            branch_gid = jsondata.get('Asset_Branch_Gid')
            drop_b = {
            "Table_name": "gal_mst_tbranch",
            "Column_1": "branch_gid,branch_name,branch_code",
            "Column_2": "",
...
def faclearence_unlock(request):
    # utl.check_authorization(request)
    if request.method == 'POST':
        jsondata = json.loads(request.body.decode('utf-8'))
        obj = view_fa.FinYear()
        ...
        jsondata['params']['json']['Params'].update(classify)
        datas = json.dumps(jsondata.get('params').get('json'))
        resp = requests.post("" + ip + "/FA_CLEARANCE", params=params, data=datas, headers=headers,
                             verify=False)
        response = resp.content.decode("utf-8")
        return HttpResponse(response)