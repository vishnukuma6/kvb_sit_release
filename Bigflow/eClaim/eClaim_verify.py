...
def tour_check(self, jsondata):
    obj_claim = meClaim.eClaim_Model()
    obj_claim.employee_gid = jsondata.get('Params').get('FILTER').get('tourgid')
    ...
    durationdays = ld_dict[0].get('durationdays')
    common.logger.error([{"durationdays": str(durationdays)}])
    if durationdays > 1:
        return False
    else:
        return True
    ...
    def eligible_amount(self, jsondata, grade, empid, tour_dtl):
        ...
        if city != "":
            ...
            if tot_days > days:
                diff = tot_days - days
                if diff > 0:
                    amount_data[len(amount_data) - 1]['days'] = int(
                        amount_data[len(amount_data) - 1].get('days')) - diff
                    ...
                    if out_data != []:
                        for i in out_data:
                            if i.get('city') == city.upper():
                                ...
                                eligible_amount = (days * amount) + int(i.get('amount')) + (
                                            int(isleave) * 0) + inspection_amount
                    else:
                        eligible_amount = (days * amount) + (int(isleave) * 0) + inspection_amount
                else:
                    days = days - (int(holiday) + int(isleave))
                    eligible_amount = (days * amount) + (holiday * amount * 1.25) + (
                                int(isleave) * 0) + inspection_amount
...
class lodging():
    def date_valid(self, jsondata, grade):
        ...
        if hour > 0:
            days = days + 1
        noofdays = days
        common.logger.error([{"noofdays": str(noofdays)}])
        tot_amount = 0
        amount_data = elig_citytoamount_effectivedate(self, grade, city, frdt, todt, expensegid, 1, "Lodge")
...
class pkg_moving():
    def validate_data(self, jsondata, grade,empid,entity_gid):
        obj_claim = meClaim.eClaim_Model()
        ...
        if out_data.get("MESSAGE") == 'FOUND':
            ...
            for d in data:
                tourreason = d.get('name')
                # tourreason = 'INSPECTION'
        if tourreason.upper() == 'TRANSFER WITH FAMILY':
            maxtonnage = ld_elig[0].get('tonnagefamily')

        else:
            maxtonnage = ld_elig[0].get('maxtonnage')

        if int(totaldisttrans) in range(0, 1000):
            rupeeforton = ld_elig[0].get('freight1000')
        else:
            rupeeforton1001 = ld_elig[0].get('freight1001')
            rupeeforton = ld_elig[0].get('freight1000')
        if int(tonnagehhgood) <= maxtonnage:
        ...
        if int(twowheelertrans) in twowheeler:
            if int(totaldisttrans) in range(0, 1000):
                ...
                if int(totaldisttrans) - int(distinhilly) != 0:
                    tot_dist = int(totaldisttrans) - int(distinhilly)
                    tot_dist1001 = tot_dist - 1000
                    tot_amt = tot_amt + (((int(tot_dist) - int(tot_dist1001)) * rupeeforton) + (tot_dist1001 * rupeeforton1001)) * tonnage
    ...
    def calck_driverbata(self, jsondata, grades):
        ...
        common.logger.error([{"designation": str(designation)}])
        if 'tourgid' in jsondata.get('Params').get('FILTER'):
            tourgid = jsondata.get('Params').get('FILTER').get('tourgid')
            out_data = elig_citytoamount(self, designation, "driverbata", expensegid, tourgid)
        else:
            out_data = elig_citytoamount_tmp(self, designation, "driverbata", expensegid)
    ...
    def calc_breakage(self, jsondata, grades):
        ...
        if 'tourgid' in jsondata.get('Params').get('FILTER'):
            tourgid = jsondata.get('Params').get('FILTER').get('tourgid')
            out_data = elig_citytoamount(self, designation, "Lumpsum", expensegid, tourgid)
        else:
            out_data = elig_citytoamount_tmp(self, designation, "Lumpsum", expensegid)
...
class localcon():
    def validate_data(self, jsondata, grad):
        ...
        if 'tourgid' in jsondata.get('Params').get('FILTER'):
            tourgid = jsondata.get('Params').get('FILTER').get('tourgid')
            out_data = elig_citytoamount(self, designation, city, expensegid, tourgid)
        else:
            out_data = elig_citytoamount_tmp(self, designation, city, expensegid)
...
class miscellaneous():
    def elegibility_data(self, data, grade):
        ...
        designation = json.loads(designation.get("DATA").to_json(orient='records'))
        designation = designation[0].get('designation')
        if 'center' in data.get('Params').get('FILTER'):
            if data.get('Params').get('FILTER').get('center') != 0 and data.get('Params').get('FILTER').get('center') != '0':
                city = data.get('Params').get('FILTER').get('center')
                expensegid = data.get('Params').get('FILTER').get('expensegid')
                # else using mobile api purpose
                if 'tourgid' in data.get('Params').get('FILTER'):
...
class local_deputation():
    def elegibility_data(self, data, grade):
        obj_claim = meClaim.eClaim_Model()
        expensegid = data.get('Params').get('FILTER').get('expensegid')
        city = data.get('Params').get('FILTER').get('visitcity')
        isleave = int(data.get('Params').get('FILTER').get('isleave'))
        from_date = data.get('Params').get('FILTER').get('fromdate')
        to_date = data.get('Params').get('FILTER').get('todate')
...
def elig_citytoamount(self, grade, city, expensegid, tourgid):
    ...
    ld_out_message = obj_claim.eClaim_citytoallowance_get()
    out_data = json.loads(ld_out_message.get("DATA").to_json(orient='records'))
    obj_claim.filter_json = json.dumps({"tourgid": tourgid})
    ld_out_message = obj_claim.eClaim_tourheader_get()
    tour_data = json.loads(ld_out_message.get("DATA").to_json(orient='records'))
    startdate = tour_data[0].get('startdate')
    sdate = startdate.split()
    sdate = sdate[0].split('-')
    day = date(int(sdate[2]), int(sdate[1]), int(sdate[0]))
    final_amount = []
    data = {}
    for j in out_data:
        if j.get('effectiveto') != '0' and j.get('effectiveto') != '00-00-0000':
            efdate = j.get('effectivefrom').split()
            etdate = j.get('effectiveto').split()

            efdate = efdate[0].split('-')
            etdate = etdate[0].split('-')

            from_d = date(int(efdate[2]), int(efdate[1]), int(efdate[0]))  # start date
            to_d = date(int(etdate[2]), int(etdate[1]), int(etdate[0]))
            if from_d <= day <= to_d:
...
def elig_citytoamount_effectivedate(self, grade, city, from_dt, to_dt, expensegid, entity_gid, exp_type):
    ...
    for i in range(delta.days + 1):
        ...
        common.logger.error([{"day": str(day)}])
        for j in out_data:
            common.logger.error([{"out_data_j": str(j)}])
            if j.get('effectiveto') != '0' and j.get('effectiveto') != '00-00-0000':
                efdate = j.get('effectivefrom').split()
                etdate = j.get('effectiveto').split()
                ...
                from_d = date(int(efdate[2]), int(efdate[1]), int(efdate[0]))  # start date
                to_d = date(int(etdate[2]), int(etdate[1]), int(etdate[0]))
                # from_d = datetime.datetime.date(j.get('effectivefrom'), '%d-%m-%Y')
                # to_d = datetime.datetime.date(j.get('effectiveto'), '%d-%m-%Y')
                if from_d <= day <= to_d:
                    if exp_type == "Dailydeim":
                        get_holiday = {
                            "Stategid": state_gid,
                    ...
                    date_a = date_a - totholiday_a + 1
                    data_a['days'] = date_a
                    data_a['holiday'] = totholiday_a
                    data_a['amount'] = j.get('amount')

            else:
                efdate = j.get('effectivefrom').split()
                efdate = efdate[0].split('-')
                from_d = date(int(efdate[2]), int(efdate[1]), int(efdate[0]))  # start date
                common.logger.error([{"from_d": str(from_d)}])
                if from_d <= day:
                    if exp_type == "Dailydeim":
                        get_holiday = {
                            "Stategid": state_gid,
                        ...
                        holiday = json.loads(ld_out_holiday.get("DATA").to_json(orient='records'))
                        totholiday_a = totholiday_a + int(holiday[0].get('Data'))
                    date_b = date_b - totholiday_b + 1
                    data_b['days'] = date_b
                    data_b['holiday'] = totholiday_b
                    data_b['amount'] = j.get('amount')
    common.logger.error([{"data_a": str(data_a)}])
    common.logger.error([{"data_b": str(data_b)}])
    if len(data_a) != 0:
        final_amount.append(data_a)
    if len(data_b) != 0:
        final_amount.append(data_b)
    common.logger.error([{"data_b": str(final_amount)}])
    return final_amount
...
def holiday_check(self, city, Entity_Gid, frdt, todt):
    obj_claim = meClaim.eClaim_Model()
    state_get = {
        "City": city.upper(),
        "Entity_Gid": Entity_Gid
    ...
    def invoice_detail(self, request, response, jsondata):
        try:
            ...
            if jsondata.get('Params').get('DETAILS').get('apptype') == "CLAIM":
                ...
                tourreason_data = obj_claim.eClaim_tourreasondata_get()
                if tourreason_data.get("MESSAGE") == 'FOUND':
                    tourreason = json.loads(tourreason_data.get("DATA").to_json(orient='records'))
                ...
                designation_data = obj_claim.eClaim_empwise_grade_get()
                if designation_data.get("MESSAGE") == 'FOUND':
                    designation = json.loads(designation_data.get("DATA").to_json(orient='records'))
                    designation = designation[0].get('designation')
                else:
                ...
                if gl_mapping.get("MESSAGE") == 'FOUND':
                    ...
                    return data
                cat_data = cat_apicall(self, request, cat_code, entity_gid)
                if cat_data.get("MESSAGE") == 'FOUND':
                    cat_gid = cat_data.get('DATA')[0].get('category_gid')
                else:
                    data = {"MESSAGE": "Catogory Data Missing"}
                    return data
                subcatogry_data = subcat_apicall(self, request, cat_gid, entity_gid)
                ...
                if cat_data.get("MESSAGE") == 'FOUND':
                    ...
                    data = {"MESSAGE": "Catogory Data Missing"}
                    return data
                subcatogry_data = subcat_apicall(self, request, advancecat_gid, entity_gid)
                if subcatogry_data.get("MESSAGE") == 'FOUND':
                    subcat_data = subcatogry_data.get('DATA')
                else:
                    data = {"MESSAGE": "SubCatogory Data Missing"}
            ...
            claim_credit = []
            paymode_data = paymode_apicall(self, request, entity_gid)
            if paymode_data.get("MESSAGE") == 'FOUND':
...
def get_subcat(self, subcatdata, cat):
    ...
    elif cat == 'Local Deputation Allowance':
