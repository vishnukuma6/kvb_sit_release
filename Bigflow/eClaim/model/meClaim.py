...
class eClaim_Model(mVariable.variable):
    ...
    def eClaim_tournoexpense_get(self):
        cursor = connections['eclaim_db'].cursor()
        parameters = (self.filter_json, '')
        cursor.callproc('sp_tourexpense_report_get', parameters)
        if cursor.description != None:
            columns = [x[0] for x in cursor.description]
            rows = cursor.fetchall()
            cursor.execute('select @_sp_tourexpense_report_get_1')
            outmsg_sp = cursor.fetchone()
            rows = list(rows)
            df_location = pd.DataFrame(rows, columns=columns)
            return {"DATA": df_location, "MESSAGE": outmsg_sp[0]}
    ...
    def eClaim_approverlist_get_galley(self):
        cursor = connection.cursor()
        parameters = (self.filter_json, '')
        cursor.callproc('sp_eClaimApprover_Get', parameters)
        if cursor.description != None:
            columns = [x[0] for x in cursor.description]
            rows = cursor.fetchall()
            cursor.execute('select @_sp_eClaimApprover_Get_1')
            outmsg_sp = cursor.fetchone()
            rows = list(rows)
            df_location = pd.DataFrame(rows, columns=columns)
            return {"DATA": df_location, "MESSAGE": outmsg_sp[0]}
    ...
    def eClaim_branchtoemp_get(self):
        cursor = connection.cursor()
        parameters = (self.filter_json, '')
        cursor.callproc('sp_Branchwise_employee_Get', parameters)
        if cursor.description != None:
            columns = [x[0] for x in cursor.description]
            rows = cursor.fetchall()
            cursor.execute('select @_sp_Branchwise_employee_Get_1')
            outmsg_sp = cursor.fetchone()
            rows = list(rows)
            df_location = pd.DataFrame(rows, columns=columns)
            return {"DATA": df_location, "MESSAGE": outmsg_sp[0]}
    ...
    def eClaim_tourreason_get(self):
        cursor = connections['eclaim_db'].cursor()
        quot = ''
        query = 'select gid,code,name,fileupload from claim_mst_ttourreason where status = 1 and entitygid = 1 '
        query = quot+query+quot
        cursor.execute(query)
        columns = [x[0] for x in cursor.description]
        rows = cursor.fetchall()
    ...
    def eClaim_file_remove_set(self):
        cursor = connection.cursor()
        parameters = (self.filter_json, '')
        cursor.callproc('sp_eClaimFiles_Remove_Set', parameters)
        cursor.execute('select @_sp_eClaimFiles_Remove_Set_1')
        out_message = cursor.fetchone()
        return {"MESSAGE": out_message[0]}
    ...
    def eClaim_emptodependent_get(self):
        cursor = connections['eclaim_db'].cursor()
        quot = ''
        query = 'SELECT depid as dependentid,depname as dependentname,deprelation as deprelation FROM claim_mst_tdependent where depempid ='
        gid = str(self.employee_gid)
        query = quot+query+gid+quot
        cursor.execute(query)
        columns = [x[0] for x in cursor.description]
    ...
    def eClaim_advance_approvedamount_set(self):
        cursor = connections['eclaim_db'].cursor()
        parameters = (self.jsondata, '')
        cursor.callproc('sp_advance_approvedamount_set', parameters)
        cursor.execute('select @_sp_advance_approvedamount_set_1')
        out_message = cursor.fetchone()
        return {"MESSAGE": out_message[0]}
    ...
    def eClaim_allowance_summary_get(self):
        cursor = connections['eclaim_db'].cursor()
        parameters = (self.filter_json, '')
        cursor.callproc('sp_allowance_get', parameters)
        if cursor.description != None:
            columns = [x[0] for x in cursor.description]
            rows = cursor.fetchall()
            cursor.execute('select @_sp_allowance_get_1')
            outmsg_sp = cursor.fetchone()
            rows = list(rows)
            df_location = pd.DataFrame(rows, columns=columns)
            return {"DATA": df_location, "MESSAGE": outmsg_sp[0]}
    ...
    def eClaim_tourrelaxation_summary_get(self):
        cursor = connections['eclaim_db'].cursor()
        parameters = (self.filter_json, '')
        cursor.callproc('sp_daterelaxation_get', parameters)
        if cursor.description != None:
            columns = [x[0] for x in cursor.description]
            rows = cursor.fetchall()
            cursor.execute('select @_sp_daterelaxation_get_1')
            outmsg_sp = cursor.fetchone()
            rows = list(rows)
            df_location = pd.DataFrame(rows, columns=columns)
            return {"DATA": df_location, "MESSAGE": outmsg_sp[0]}
    ...
    def eClaim_tourrelaxation_set(self):
        cursor = connections['eclaim_db'].cursor()
        parameters = (self.jsondata, '')
        cursor.callproc('sp_daterelaxation_set', parameters)
        cursor.execute('select @_sp_daterelaxation_set_1')
        out_message = cursor.fetchone()
        return {"MESSAGE": out_message[0]}

    def eClaim_allowance_set(self):
        cursor = connections['eclaim_db'].cursor()
        parameters = (self.jsondata, '')
        cursor.callproc('sp_allowance_set', parameters)
        cursor.execute('select @_sp_allowance_set_1')
        out_message = cursor.fetchone()
        return {"MESSAGE": out_message[0]}

    def eClaim_tourheader_get(self):
        cursor = connections['eclaim_db'].cursor()
        parameters = (self.filter_json, '')
        cursor.callproc('sp_tourheader_get', parameters)
        if cursor.description != None:
            columns = [x[0] for x in cursor.description]
            rows = cursor.fetchall()
            cursor.execute('select @_sp_tourheader_get_1')
            outmsg_sp = cursor.fetchone()
            rows = list(rows)
            df_location = pd.DataFrame(rows, columns=columns)
            return {"DATA": df_location, "MESSAGE": outmsg_sp[0]}
    ...
    def eClaim_localdeputation_get(self):
        cursor = connections['eclaim_db'].cursor()
        parameters = (self.filter_json, '')
        cursor.callproc('sp_localdeputation_get', parameters)
        if cursor.description != None:
            columns = [x[0] for x in cursor.description]
            rows = cursor.fetchall()
            cursor.execute('select @_sp_localdeputation_get_1')
            outmsg_sp = cursor.fetchone()
            rows = list(rows)
            df_location = pd.DataFrame(rows, columns=columns)
            return {"DATA": df_location, "MESSAGE": outmsg_sp[0]}
    ...
    def eClaim_localdeputation_set(self):
        cursor = connections['eclaim_db'].cursor()
        parameters = (self.jsondata, self.jsonData,'')
